...
Define districts & year ranges. Four Cologne districts are configured with bounding boxes and plausible construction year ranges (e.g. Altstadt 1850‑1950, Ehrenfeld 1920‑1980). Each district has a limit on the number of buildings to fetch
github.com
.

Fetch building footprints. For each district, the script queries the Overpass API and converts the returned OSM ways into a FeatureCollection. Only polygons representing buildings are retained
github.com
.

Assign synthetic ages. For each building, the script calculates a centroid and then picks a random year within the district’s configured range
github.com
. Metadata is added (district name, synthetic year, year range, centroid coordinates) and the features are stored.

Export results. All enriched features are saved to a GeoJSON file (synthetic_building_age.geojson) and a CSV file (synthetic_building_age.csv)
github.com
. The CSV uses simple columns (id, district, year_built, year_range) for easier inspection or plotting.
...
Prerequisites. Install Python 3.11+ and ensure you can access the internet (the script queries the Overpass API). No third‑party packages are required.

Run from the command line. From the root of the repository:
...
Outputs. After running, you will find synthetic_building_age.geojson and synthetic_building_age.csv in the same folder. These files will be overwritten each time you run the script.

Customisation. You can adjust the districts or year ranges by editing the DISTRICTS list at the top of the script
github.com
. You can also change the limit per district to fetch more or fewer buildings.
...
Displays a dark basemap and overlays building footprints coloured by year of construction. Colours correspond to defined year classes (e.g. 1850–1900, 1930–1950, etc.)
github.com
. Buildings without a known year are coloured dark and labelled as “synthetic/unknown” in the legend
github.com
.
...
Building footprints: retrieved live from OpenStreetMap via the Overpass API. OSM data is © OpenStreetMap contributors and licensed under the Open Database License
. When exporting or publishing derived data, be sure to attribute OSM and respect the licence.

Synthetic years: randomly generated by this project for illustration only. They do not reflect actual construction dates.

If you find errors or have suggestions, feel free to open an issue or pull request in the 30DayMapChallenge‑2025
 repository.
